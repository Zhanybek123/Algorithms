...
// MARK: -Same Tree (Practice, My own approach)
...
// MARK: -Same Tree (Practice, My own approach without last check for nil)
//
//class Solution {
//    func isSameTree(_ p: TreeNode?, _ q: TreeNode?) -> Bool {
//        if p == nil && q == nil {
...
//MARK: -Binary Tree Level Order Traversal (Practice)
...
//MARK: -Binary Tree Level Order Traversal (Practice BFS)
...
// MARK: -Validate Binary Search Tree (Practice inordered traversal) Cleaned up
...
//        
//        var current = node.val
//        if let low = low, low >= current { return false }
//        if let high = high, high <= current { return false }
//
//        if !isBST(node.right, node.val, high) { return false }
//        return true
//    }
...
//
//        // Recursively call maxGain for left and right subtrees
//        let leftGain = max(maxGain(node.left), 0)
//        let rightGain = max(maxGain(node.right), 0)
//
//        // The path that passes through the node and both children
